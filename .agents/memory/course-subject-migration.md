...
- **OpenAPI example strings carry the subject too:** `lib/api-spec/openapi.yaml` has illustrative `description:` examples (e.g. NextProblemInput's "give me harder X problems") that bake in old-subject vocabulary and silently propagate into the generated `lib/api-zod` + `lib/api-client-react` files. Grep the spec for example phrasing, fix it, then re-run `pnpm --filter @workspace/api-spec run codegen` so the generated files update.
